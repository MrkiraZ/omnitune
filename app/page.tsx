...
export default function Home() {
  ...
  const acceptedIndexRef = useRef(-1)
  const releaseRef = useRef(true)
  const acceptLockUntilRef = useRef(0)
  ...
  const registerNote = useCallback((note: string) => {
    ...
    if (isHit && acceptedIndexRef.current !== eventIndex) {
      acceptedRef.current = expected
      acceptedIndexRef.current = eventIndex
      acceptLockUntilRef.current = performance.now() + 700
  ...
  const analyze = useCallback(() => {
    ...
    if (!result.root) { stableRef.current = ""; stableCountRef.current = 0; acceptedRef.current = ""; releaseRef.current = true; setStatus("Sem sinal — toque uma corda"); setDetected("—") }
      ...
      if (stableCountRef.current >= 10 && releaseRef.current && acceptedIndexRef.current !== eventIndex && performance.now() >= acceptLockUntilRef.current) { releaseRef.current = false; registerNote(result.root) }
  ...
  const reset = () => { setEventIndex(0); setScore(0); setCombo(0); setCorrect(0); setAttempts(0); setDetected("—"); setFeedback("Toque a nota indicada para avançar"); acceptedRef.current = ""; acceptedIndexRef.current = -1; releaseRef.current = true; acceptLockUntilRef.current = 0 }
  const chooseTrack = (nextTrack: Track) => { stopListening(); setTrack(nextTrack); setEventIndex(0); setDetected("—"); setScore(0); setCombo(0); setCorrect(0); setAttempts(0); acceptedRef.current = ""; acceptedIndexRef.current = -1; releaseRef.current = true; acceptLockUntilRef.current = 0; setFeedback(`Faixa: ${nextTrack.title}`); setLibraryOpen(false) }
  ...
  return <main className="min-h-screen bg-[#090912] text-foreground">
    ...
    <section className="mx-2 mt-2 overflow-hidden rounded-2xl border border-[#302552] bg-[#0b0b15] sm:mx-4"><div className="flex items-center justify-between border-b border-[#252443] px-3 py-2"><div><span className="font-mono text-[10px] text-cyan-300">PISTA DE TREINO</span><p className="text-xs text-muted-foreground">Alvo único: <strong className="text-foreground">{current.label}</strong> <span className="mx-2 text-cyan-300">·</span> próxima sugestão no braço: <strong className="text-cyan-300">{next?.label || "fim"}</strong></p></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${flash === "hit" ? "bg-primary text-primary-foreground" : flash === "miss" ? "bg-destructive text-white" : "bg-white/10"}`}>{flash === "hit" ? "ACERTO" : flash === "miss" ? "OUVIDA, MAS NÃO É O ALVO" : status}</span></div><div className="relative h-24 bg-[linear-gradient(90deg,transparent_24%,#26233d_25%,transparent_26%,transparent_49%,#26233d_50%,transparent_51%,transparent_74%,#26233d_75%,transparent_76%)]"><div className={`absolute left-[12%] top-8 grid h-9 w-24 place-items-center rounded-full border-2 text-sm font-black transition-none ${current.color === "green" ? "border-lime-300 bg-lime-300 text-slate-950" : current.color === "red" ? "border-red-400 bg-red-400 text-slate-950" : current.color === "yellow" ? "border-yellow-300 bg-yellow-300 text-slate-950" : "border-blue-300 bg-blue-300 text-slate-950"} ${flash === "hit" ? "ring-4 ring-lime-300/70" : ""}`}><span>{current.label}</span><small className="ml-1 text-[8px]">ALVO</small></div><div className="absolute right-[12%] top-8 hidden h-9 w-24 place-items-center rounded-full border border-cyan-300/70 bg-cyan-400/10 text-xs font-bold text-cyan-200">{next?.label || "FIM"}<small className="ml-1 text-[8px]">PRÓXIMA</small></div></div><div className="h-2 bg-[#1c1930]"><div className="h-full bg-primary transition-all" style={{ width: `${progress}%` }} /></div></section>
